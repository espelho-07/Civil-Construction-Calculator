...
const BM_GRADINGS = {
    'grading-1': {
        title: 'Bituminous Macadam Grading-I (Nominal max aggregate size 40mm)',
        sieves: ['45 mm', '37.5 mm', '26.5 mm', '19 mm', '9.5 mm', '4.75 mm', '2.36 mm', '0.3 mm', '0.075 mm', 'Pan'],
        limits: { '45 mm': [100, 100], '37.5 mm': [90, 100], '26.5 mm': [75, 100], '19 mm': [35, 80], '9.5 mm': [15, 55], '4.75 mm': [0, 35], '2.36 mm': [0, 25], '0.3 mm': [0, 10], '0.075 mm': [0, 5] }
    },
    'grading-2': {
        title: 'Bituminous Macadam Grading-II (Nominal max aggregate size 19mm)',
        sieves: ['26.5 mm', '19 mm', '9.5 mm', '4.75 mm', '2.36 mm', '0.3 mm', '0.075 mm', 'Pan'],
        limits: { '26.5 mm': [100, 100], '19 mm': [90, 100], '9.5 mm': [35, 70], '4.75 mm': [0, 40], '2.36 mm': [0, 25], '0.3 mm': [0, 10], '0.075 mm': [0, 5] }
    }
};
...
export default function BituminousMacadamCalculator() {
    ...
    const currentGrading = BM_GRADINGS[grading];
    ...
    return (
        <main className="min-h-screen bg-[#F7F9FF]">
            ...
            <div className="max-w-6xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-12 items-start">
                <div>
                    <h1 className="text-3xl font-bold text-[#0A0A0A] mb-2">Bituminous Macadam (BM)</h1>
                    ...
                    <section className="mb-8">
                        ...
                        <div className="bg-white rounded-xl p-6 border border-[#e5e7eb]">
                            <p className="text-[#0A0A0A] leading-relaxed mb-4">
                                Bituminous Macadam (BM) is an open-graded mix with coarse aggregates bound with bitumen. It is used as a binder course or base course in flexible pavement construction.
                            </p>
                            <p className="text-[#0A0A0A] leading-relaxed">
                                BM has higher air voids compared to dense mixes and is typically used where a thick base layer is required. Two gradings are available: <strong>Grading-I (40mm)</strong> and <strong>Grading-II (19mm)</strong>.
                            </p>
                        </div>
                    </section>
                    ...
                    <section className="mb-8">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {Object.entries(BM_GRADINGS).map(([key, g]) => (
                                <div key={key} onClick={() => setGrading(key)} className={`bg-white rounded-xl p-4 border-2 cursor-pointer transition-all ${grading === key ? 'border-[#3B68FC] shadow-lg' : 'border-[#e5e7eb] hover:border-[#3B68FC]'}`}>
                                    ...
                                    <div className="text-center text-xs text-[#6b7280]">{key === 'grading-1' ? '40mm Nominal Max' : '19mm Nominal Max'}</div>
                                </div>
                            ))}
                        </div>
                    </section>
                    ...
                    {results && (
                        <section className="mb-8">
                            ...
                            <div className="bg-white rounded-xl p-6 border border-[#e5e7eb] overflow-x-auto">
                                <table className="w-full text-sm">
                                    <thead><tr className="bg-gradient-to-r from-gray-50 to-slate-100"><th className="border px-3 py-2">Sieve</th><th className="border px-3 py-2">Retained (g)</th><th className="border px-3 py-2">Cum. Retained %</th><th className="border px-3 py-2">% Passing</th><th className="border px-3 py-2">Status</th></tr></thead>
                                    <tbody>
                    ...
                    <section className="mb-8">
                        ...
                        <div className="bg-white rounded-xl p-6 border border-[#e5e7eb]">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {[
                                    { title: 'Aggregate Quality', desc: 'Angular, crushed stone', icon: 'fa-gem' },
                                    { title: 'Bitumen Grade', desc: 'VG-10 or VG-30', icon: 'fa-tint' },
                                    { title: 'Layer Thickness', desc: '50-100mm compacted', icon: 'fa-ruler-vertical' },
                                    { title: 'Temperature', desc: '120-160°C mixing temp', icon: 'fa-thermometer-half' },
                                ].map((item) => (
                ...
                <aside ref={sidebarRef} className="sticky top-20 h-fit">
                    <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] overflow-hidden border border-[#e5e7eb]">
                        <div className="px-5 py-4 border-b border-[#e5e7eb] bg-gradient-to-r from-gray-50 to-slate-100 flex items-center gap-3">
                            <i className="fas fa-fill-drip text-xl text-gray-700"></i>
                            <h2 className="font-semibold text-[#0A0A0A]">BM Grading Calculator</h2>
                        </div>
                        <div className="p-4">
