            ...
            <div className="max-w-6xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-12 items-start">
                <div>
                    <div className="flex items-center justify-between mb-2">
                        <h1 className="text-3xl font-bold text-[#0A0A0A]">Vane Shear Test Calculator <span className="text-sm font-normal text-gray-500">IS: 2720 (Part 30)</span></h1>
                        <CalculatorActions
                            calculatorSlug="vane-shear"
                            calculatorName="Vane Shear Calculator"
                            calculatorIcon="fa-fan"
                            category="Soil Test"
                            inputs={{ springConstant, tests }}
                            outputs={results || {}}
                        />
                    </div>
                            ...
                            <div className={`${theme.bgLight} rounded-xl p-4`}>
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="text-center">
                                        ...
                                        <div className={`text-lg font-bold ${theme.text}`}>{results?.avgShearStrength} Kg/cm²</div>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-xs text-gray-500">Avg. Cohesion (C<sub>u</sub>)</div>
                                        <div className={`text-lg font-bold ${theme.text}`}>{results?.avgCohesion} Kg/cm²</div>
                                    </div>
                                </div>
                                <div className="mt-3 text-xs">
                                    {results?.tests?.map((t, i) => (
                                        <div key={i} className="flex justify-between bg-white px-2 py-1 rounded mb-1">
                                            <span>Test-{i + 1}</span>
                                            <span className={`font-bold ${theme.text}`}>{t.cohesion} Kg/cm²</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
