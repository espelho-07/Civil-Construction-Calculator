...
const BC_GRADINGS = {
    'grading-1': {
        title: 'Bituminous Concrete Grading-I (Nominal maximum aggregate size 19mm)',
        sieves: ['26.5 mm', '19 mm', '13.2 mm', '9.5 mm', '4.75 mm', '2.36 mm', '1.18 mm', '0.6 mm', '0.3 mm', '0.15 mm', '0.075 mm', 'Pan'],
        limits: { '26.5 mm': [100, 100], '19 mm': [90, 100], '13.2 mm': [59, 79], '9.5 mm': [52, 72], '4.75 mm': [35, 55], '2.36 mm': [28, 44], '1.18 mm': [20, 34], '0.6 mm': [15, 27], '0.3 mm': [10, 20], '0.15 mm': [5, 13], '0.075 mm': [2, 8] }
    },
    'grading-2': {
        title: 'Bituminous Concrete Grading-II (Nominal maximum aggregate size 13.2mm)',
        sieves: ['19 mm', '13.2 mm', '9.5 mm', '4.75 mm', '2.36 mm', '1.18 mm', '0.6 mm', '0.3 mm', '0.15 mm', '0.075 mm', 'Pan'],
        limits: { '19 mm': [100, 100], '13.2 mm': [90, 100], '9.5 mm': [70, 88], '4.75 mm': [53, 71], '2.36 mm': [42, 58], '1.18 mm': [34, 48], '0.6 mm': [26, 38], '0.3 mm': [18, 28], '0.15 mm': [12, 20], '0.075 mm': [4, 10] }
    }
};
...
export default function BituminousConcreteCalculator() {
    ...
    const currentGrading = BC_GRADINGS[grading];
    ...
    return (
        <main className="min-h-screen bg-[#F7F9FF]">
            ...
            <div className="max-w-6xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-12 items-start">
                <div>
                    <h1 className="text-3xl font-bold text-[#0A0A0A] mb-2">Bituminous Concrete (BC) Grading</h1>
                    ...
                    <section className="mb-8">
                        ...
                        <div className="bg-white rounded-xl p-6 border border-[#e5e7eb]">
                            <p className="text-[#0A0A0A] leading-relaxed mb-4">
                                Bituminous Concrete (BC) is a premium quality wearing course used on high-traffic roads. It consists of well-graded aggregates bound with bituminous binder, providing excellent durability, skid resistance, and surface texture.
                            </p>
                            <p className="text-[#0A0A0A] leading-relaxed">
                                BC is the topmost layer of flexible pavement and is placed over DBM (Dense Bituminous Macadam). Two gradings are available: <strong>Grading-I (19mm)</strong> for normal wearing course and <strong>Grading-II (13.2mm)</strong> for thin wearing course applications.
                            </p>
                        </div>
                    </section>
                    ...
                    <section className="mb-8">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {Object.entries(BC_GRADINGS).map(([key, g]) => (
                                <div key={key} onClick={() => setGrading(key)} className={`bg-white rounded-xl p-4 border-2 cursor-pointer transition-all ${grading === key ? 'border-[#3B68FC] shadow-lg' : 'border-[#e5e7eb] hover:border-[#3B68FC]'}`}>
                                    ...
                                    <div className="text-center text-xs text-[#6b7280]">{key === 'grading-1' ? '19mm Nominal Max' : '13.2mm Nominal Max'}</div>
                                </div>
                            ))}
                        </div>
                    </section>
                    ...
                    <section className="mb-8">
                        ...
                        <div className="bg-white rounded-xl p-6 border border-[#e5e7eb]">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {[
                                    { title: 'Aggregate Quality', desc: 'Use clean, crushed aggregates', icon: 'fa-gem' },
                                    { title: 'Bitumen Grade', desc: 'VG-30 or VG-40 as per traffic', icon: 'fa-tint' },
                                    { title: 'Layer Thickness', desc: '30-40mm for wearing course', icon: 'fa-ruler-vertical' },
                                    { title: 'Temperature Control', desc: 'Mixing at 150-170°C', icon: 'fa-thermometer-half' },
                                    { title: 'Compaction', desc: 'Min 97% of Marshall density', icon: 'fa-compress-arrows-alt' },
                                    { title: 'Surface Finish', desc: 'Smooth, free from segregation', icon: 'fa-check-circle' },
                                ].map((item) => (
                ...
                <aside ref={sidebarRef} className="sticky top-20 h-fit">
                    <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] overflow-hidden border border-[#e5e7eb]">
                        <div className="px-5 py-4 border-b border-[#e5e7eb] bg-gradient-to-r from-blue-50 to-indigo-50 flex items-center gap-3">
                            ...
                            <h2 className="font-semibold text-[#0A0A0A]">BC Grading Calculator</h2>
                        </div>
                        <div className="p-4">
